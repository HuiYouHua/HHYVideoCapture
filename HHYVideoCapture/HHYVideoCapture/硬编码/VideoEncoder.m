...
- (void)setupFileHandle {
    // 1.获取沙盒路径
    NSString *file = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"硬编码.h264"];
...
- (void)setupVideoSession {
    ...
    // 6.设置码率(码率: 编码效率, 码率越高,则画面越清晰, 如果码率较低会引起马赛克 --> 码率高有利于还原原始画面,但是也不利于传输)(单位时间内保存的数据量)
...
void didCompressH264(void *outputCallbackRefCon, void *sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer) {
    ...
    // 判断当前帧是否为关键帧
    // 获取sps & pps数据
    // SPS（Sequence Parameter Set）：序列参数集,保存整个序列的相关信息
    // PPS（Picture Parameter Sets）：图像参数集,保存I帧相关信息
    if (isKeyframe)
    {
